  ...
  */
  ...
  VariantResponse APIHandler<Requester>::PlayerMove()
  {
   // std::cout << "I AM PLAYERMIVE" << std::endl;
    ...
    //  std::cout<<"->"<<req_[HttpHeader::content_type]<<"<-"<<std::endl;
    ...
    try
    {
      ...
      if(player == nullptr) return Make401JSB(req_.version(), req_.keep_alive(),
           ...
           auto dogspeed = GetDogSpeedByDrection(move, sess_spd);
          
          
          dog->SetDogSpeedAndDirection(std::move(dogspeed), model::string_nswe.at(move));
        //  std::cout<<dog->GetOrientation()<<"  "<<dog->GetDodSpd()<<std::endl;

      
      return Make200JSB(req_.version(), req_.keep_alive(), "{}");
    }
    catch (...)
    {
      // ОТВЕТ В СЛУЧАЕ ОШИБКИ
