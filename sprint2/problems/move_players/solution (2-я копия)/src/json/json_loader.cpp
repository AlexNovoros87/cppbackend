...
namespace json_loader
{
    ...
    api::Play LoadPlay(const std::filesystem::path& json_path){
         ...
         auto maps = ParseMaps(parse.as_object().at(Literals::MAPS).as_array());
         LoadMapsToGame(maps, game);
         
         
         if(parse.as_object().count(Literals::defaultDogSpeed)>0){
          double default_speed = parse.as_object().at(Literals::defaultDogSpeed).as_double();
          return api::Play(std::move(game), default_speed);
         }
         
        api::Play play(std::move(game));
        return play;
 };
