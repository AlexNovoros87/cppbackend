...
std::vector<std::string> ParseToken(std::string target){
    ...
    // while (target[i] == ' ') ++i; 
    // if(i != 0)target = target.substr(i);
    
    std::vector<std::string> parsed;
    std::string tmp;

    for(int j = 0; j<target.size(); ++j){
      if(target[j] != ' '){ tmp += target[j];}
      else{
      ...
      if(!tmp.empty()) parsed.push_back(tmp);   
      if(parsed[0] != "Bearer" || parsed[1].size() != 32) return {};
      return parsed;
...
DirectionAPI Director(const std::vector<std::string> &container){
  if(container.size() < 3 || container.size() > 4 ) return DirectionAPI::ERROR;
  if(container[2] == json_foo::maps) return DirectionAPI::MAPS;
  if(container[2] != json_foo::game) return DirectionAPI::ERROR;
  if(container.size() == 4){
    if(container[3] == json_foo::players) return DirectionAPI::PLAYERS;
    if(container[3] == json_foo::join) return DirectionAPI::JOIN;
  }
...
request_handler::TypeRequest GetTypeRequset(const std::vector<std::string> &vec)
  ...
  bool CheckBaseValid(const std::vector<std::string> &container)
  {
    if (container.size() < 3 || container.size() > 4)
      return false;
    if (container[0] != json_foo::api) return false;
    if (container[1] != json_foo::v1)  return false;
...
namespace request_handler
{
  ...
  StringResponse Template(unsigned version, bool alive, bool control){
   StringResponse resp;
    ...
    if(control){resp.set(HttpHeader::cache_control, std::string(req_static_str::no_cache));}
    return resp;
};
