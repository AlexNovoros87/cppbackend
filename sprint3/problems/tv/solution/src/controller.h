...
private:
    ...
    [[nodiscard]] bool ShowInfo(std::istream &input, std::ostream &output) const
    {
        ...
        if (EnsureNoArgsInInput(INFO_COMMAND, input, output))
        {
            if (tv_.IsTurnedOn())
            {
                ...
                std::cout << "Channel number is " << *(tv_.GetChannel()) << std::endl;
            }
            else
            {
    ...
    {
        bool access = true;
        std::string try_channel;
        input >> try_channel;

        if (try_channel.empty())
            access = false;
        if (access)
        {
            for (size_t i = 0; i < try_channel.length(); ++i)
            {
                if (!isdigit(try_channel[i]))
                    access = false;
            }
        }
        ...
        try{
         int channel = std::stoi(try_channel);
